...
for method in ['udo', 'vces']:
    ...
    mesh = Mesh(ngmsh)  # there seems to be no mesh.deep_copy() or similar
    for i in range(refinements + 1):
        # get initial iterate from previous mesh (or zero when i==0)
        print(f"{method} level {i}")
        ...
        print(f"  L2 error norm = {errornorm(u, gbdry):.3e}")
        if i == refinements:
            break
        ...
        if debugoutputs:
            VTKFile(f"result_{method}{i}.pvd").write(u, lbound, mark)
    ...
    VTKFile(f"result_{method}.pvd").write(u, lbound, uerror)
