...
class AdaptiveVISolver:
    def __init__(self, m0=32, T=1.0, dt=0.01, target_complexity=100.0, h_min=1.0e-6, h_max=1):
        ...
        # Hemisphere obstacle parameters
        self.r0 = 0.9
        self.psi0 = np.sqrt(1.0 - self.r0 * self.r0)
        self.dpsi0 = -self.r0 / self.psi0
        self.initial_offset = Constant(-1)  # Start below zero
        # Adjust to control how fast obstacle rises (reaches 0 at t=1)
        self.raise_rate = Constant(-.5*self.initial_offset / self.T)

        # Mesh and adaptation
        self.mesh = RectangleMesh(
            m0, m0, Lx=1.0, Ly=1.0, originX=-1.0, originY=-1.0) 
        self.amr = VIAMR()
        ...
        # Output file
        self.outfile = VTKFile(
            "result_zero_source_timestep_obstacle_buffered.pvd", adaptive=True)
    ...
    def update_obstacle(self):
        """Update the time-dependent obstacle (rising hemisphere)."""
        x, y = SpatialCoordinate(self.mesh)
        r = sqrt(x**2 + y**2)

        # Hemisphere shape (smooth transition at r0)
        psi_ufl = conditional(
            le(r, self.r0),
            sqrt(1.0 - r * r),
            self.psi0 + self.dpsi0 * (r - self.r0)
        )

        # Vertical offset that increases with time
        # At t=0: offset = -1, obstacle is below zero
        # At t=T: offset approaches 0 (or customize as needed)
        time_offset = self.initial_offset + self.raise_rate * self.t_param

        # Final obstacle: hemisphere + time-dependent vertical shift
        obstacle_ufl = psi_ufl + time_offset

        self.lb = Function(self.V, name="obstacle").interpolate(obstacle_ufl)
    ...
    def solve_step(self, replace_u_n=False, update_t=False, write=False):
        ...
        """
        # Update obstacle for current time
        self.update_obstacle()

        # Weak form using backward Euler with zero source term
        v = TestFunction(self.V)
        F = ((self.u - self.u_n) / self.dt * v +
             inner(grad(self.u), grad(v))) * dx

        # Set up the VI problem
        problem = NonlinearVariationalProblem(F, self.u, self.bcs)
        solver = NonlinearVariationalSolver(
            problem, solver_parameters=self.solver_params)

        # Time-dependent obstacle constraint: u >= lb(t)
        ...
        if write:
            ...
            self.outfile.write(self.u, self.lb, active_set, time=self.t)
    ...
    def get_metric(self):
        ...
        metric = self.amr.adaptaveragedmetric(
            self.mesh, self.u, self.lb, return_metric=True)
        return metric
    ...
    def run_with_buffer(self, buffer_size=10):
        ...
        for step in range(self.num_steps + 1):
            ...
            if step == 0:
                ...
                self.update_obstacle()
                self.mesh = self.amr.adaptaveragedmetric(
                    self.mesh, self.u, self.lb)
            ...
            if step % buffer_size == 0:
                ...
                for _ in range(buffer_size):
                    self.solve_step(replace_u_n=True,
                                    update_t=False, write=False)
                    metric, vcdsmooth = self.amr.adaptaveragedmetric(
                        self.mesh, self.u_n, self.lb, metric=True, gamma=.95, bracket=[0.1, 0.9], smooth=True)
    ...
    def run_simple(self):
        ...
        for step in range(self.num_steps + 1):
            ...
            # Setup for next step
            prev_u_n = self.u_n.copy(
                deepcopy=True) if self.u_n is not None else None
