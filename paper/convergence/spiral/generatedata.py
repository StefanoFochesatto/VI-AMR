...
vcesAdaptVertexCounts = [33, 95, 267, 700, 1572, 3287, 6788,33]
...
if __name__ == "__main__":
    ...
    problem_instance = SpiralObstacleProblem(TriHeight=args.initTriHeight)
    amr_instance = VIAMR()
    ...
    with CheckpointFile("ExactSolutionSpiral.h5", "r") as afile:
    ...
    for i in range(args.maxIterations):
        ...
        if method == methodlist[0]:  # vcd
            ...
            mark = amr_instance.vcdmark(u, lb)
            ...
            mtic = time.perf_counter()
            mark = amr_instance.udomark(u, lb, n=1)
            mtoc = time.perf_counter()
            rtic = time.perf_counter()
            mesh = amr_instance.refinemarkedelements(mesh, mark)
            rtoc = time.perf_counter()
        ...
        elif method == methodlist[4]:  # vcd + br on inactive set
            mtic = time.perf_counter()
            markFB = amr_instance.vcdmark(u, lb)
            resUFL = -div(grad(u))
        ...
        elif method == methodlist[5]:  # udo + br on inactive set
            mtic = time.perf_counter()
            markFB = amr_instance.udomark(u, lb, n=1)
            resUFL = -div(grad(u))
            (markBR, _, _) = amr_instance.brinactivemark(u, lb, resUFL, theta=0.5)
            mark = amr_instance.unionmarks(markFB, markBR)
            mtoc = time.perf_counter()
        ...
        elif method == methodlist[6]:  # uniform
