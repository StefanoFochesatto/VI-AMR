...
This completes our \emph{a priori} considerations for FE obstacle problem \eqref{eq:fe:vi}.

FIXME DEFINE BLISTERING PROPERTY AND NOTE IT IS UNIVERSAL FOR SMOOTH $\psi$ UNILATERAL PROBLEMS VIA SUBSTITUTION $w=u-\psi$; SINCE $A_u \subset \{f<0\}$, NO FURTHER SOLUTION IS NEEDED FOR ELEMENTS WHICH ARE CURRENTLY ACTIVE AND WHICH ARE CONTAINED IN $\{f<0\}$

In Sections \ref{sec:viamr}--\ref{sec:app} we will also exploit an \emph{a posteriori} analysis using an error indicator for the PDE problem within the approximate inactive set.  While our application of this idea to VI problems is heuristically driven, the error indicator itself is presented here as an example for the classical Poisson problem with residual $r(v)=-\nabla^2 v - f$.
