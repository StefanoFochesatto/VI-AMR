...
\section{Pragmatic \emph{a posteriori} considerations for obstacle problems} \label{sec:aposteriori}

Recall that for a solution to VI \eqref{eq:vi}, the PDE which applies in the inactive set $I_u$ is called the \emph{interior condition} \cite{KinderlehrerStampacchia1980}.  In Section \ref{sec:results} we will demonstrate strategies which exploit an interior-condition \emph{a posteriori} error indicator if it is available.  Such an error indicator is well-known for the classical Poisson problem.

FIXME: EVEN SIMPLER GRADIENT-BASED ESTIMATOR \cite[Chapter 4]{AinsworthOden2000}, BECAUSE THAT CAN BE USED IN Section \ref{sec:app}

\begin{example}  \label{example:br}  Suppose $u \in H^1(\Omega)$ solves the weak form PDE $a(u,v) = \int_\Omega f v\,dx$ for all $v \in H^1_0(\Omega)$, with $u=g$ on $\partial \Omega$, for the bilinear form $a(u,v)=\int_\Omega \grad u\cdot \grad v\,dx$.  Suppose $u_h$ from $P_1$ solves the corresponding finite-dimensional weak form.  For each element $K \in \cT_h$, define $n_K$ as the unit outward normal vector on $\partial K$.  For a pair of elements $L,K$ incident to an edge $\gamma$, and any vector field $Z$ with traces $Z_L,Z_K$ on $\gamma$, let $\left\llbracket Z \cdot n \right\rrbracket = Z_L \cdot n_L + Z_K \cdot n_K$ be the jump of $Z$ on $\gamma$.  Given the residual $r(v)=-\nabla^2 v - f$ defined within the elements, the Babu\v{s}ka--Rheinboldt (BR) \cite{BabuskaRheinboldt1979} error estimator is
...
The local element-wise error estimators, $\eta_K$ in Example \ref{example:br}, can be thresholded or sorted to providing tagging of elements for refinement.  Section 4.2 of \cite{BangerthRannacher2003}, for example, suggests strategies for element tagging based on local estimators.  In Section \ref{sec:results} we show examples where all (computed) inactive elements satisfying $\eta_K \ge \theta \max \eta_K$, for $0<\theta<1$, are tagged and refined.

The BR error estimator in Example \ref{example:br}, for the Poisson equation, is explicit and easily-computed.  However, for nonlinear operators $F$, a convenient error estimator may not even be known; for example, no estimator is know for the problem in Section \ref{sec:app}.  The approach in \cite{AinsworthOdenLee1993} extends the BR estimator to certain obstacle problems, but with heuristic aspects.  An alternative to a local BR-type estimator is provided by the \emph{dual weighted residual} (DWR) technique \cite{BangerthRannacher2003}.  Such estimators use nonlocal weights, essentially Green's functions of the corresponding PDE.  Suttmeier \cite{Suttmeier2008} has extended the DWR technique to VI problems, but again with heuristics needed even for the classical obstacle problem.

At least for obstacle problems with disconnected inactive sets---see for example Figure \ref{fig:activesizes} right, useful weightings from a DWR technique are limited in range by the free boundary defining the component of the inactive set.  Furthermore, the weights computed in a DWR technique must necessarily be fragile components of an AMR approach to a problem like the one with a spiral active set in Figure \ref{fig:activesizes} left.  In this paper we avoid the complexity of computing the VI-type and weighted error estimators of \cite{AinsworthOdenLee1993} and \cite{Suttmeier2008}, and instead take the more pragmatic approach of combining fast refinement near a computed free boundary with application of an inexpensive BR-type estimator within the computed inactive sets.

Returning to the continuum problems \eqref{eq:vi}, we point out a qualitative property of certain unilateral obstacle problems, which can sometimes be exploited \emph{a posteriori} for fast AMR.  Note that for classical problems \eqref{eq:classical:vi} with continuous sources $f\in C(\bar\Omega)$, in general $f(x)>0$ can occur in the active set.  That is, the force may be locally upward, yet insufficient to lift $u$ above a concave $\psi$.  In certain reasonably-common situations this does not occur.
...
By definition, we say that a unilateral obstacle problem, based on $\ell[v] = \int_\Omega fv\,dx$ with $f\in C(\bar \Omega)$, has the \emph{blistering property} if $A_u \subset \{x \in \Omega\, :\, f(x)\le 0\}$ \cite{JouvetBueler2012}, that is, if the conclusion of Lemma \ref{lem:blister} holds.  To explain this language, the contrapositive says that if $f(x)>0$ anywhere then the inactive set $I_u$ must be non-empty, and $x\in I_u$; even a small upward force ``blisters'' the membrane off the obstacle.
...
For blistering-property problems, substantial efficiency can be achieved by avoiding refinement in the computed active sets $A_u^h$, but elements where no refinement is necessary must be carefully identified.  Assume $K\in \cT_h$, $K \subset A_u^h$, and $\psi=0$.  The degenerate case $f=0$ must be excluded within $K$, and so we must confirm that $K$ is contained in the set $\Omega_- = \{x\in \Omega\,:\,f(x) < 0\}$; note that $\Omega_-$ may be regarded as part of the data of the problem.  Then $K$ can be omitted from marking.  However, if later refinements move the free boundary to impinge on $K$ then  the benefit is lost, so generally we mark active-set elements which are near $\Gamma_u^h$ for refinement.
