...
The above example, and our numerical results (Section \ref{sec:results}), justifies our approach to adaptive mesh refinement (AMR) for VIs.

Three AMR methods are described in Section \ref{sec:viamr}.  Two methods are of the tag-and-refine type [FIXME: CITE?], only differing by the analysis technique used to identify which elements to tag.  For these methods skeleton-based refinement \cite{PlazaCarey2000} is applied, as implemented in PETSc's DMPlex component \cite{petsc-user-ref}.  A third method is goal-oriented and metric-based \cite{Alauzet2010,Wallworketal2020}, where an intermediate representation of the size, shape, and orientation of the refined mesh is constructed, namely a tensor-valued Riemannian metric.  For this third method the mesh is refined, from the metric, using the MMG mesh library \cite{DapognyDobrzynskiFrey2014}.
