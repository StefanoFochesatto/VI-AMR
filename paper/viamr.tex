...
The methods of the VIAMR library, documented in Section \ref{sec:viamr}, use such \emph{a posteriori} information to estimate the errors made in certain elements, or to (heuristically) identify elements near free boundaries where refinement should reduce location errors in the sets in \eqref{eq:fe:sets}.  Adaptive mesh refinement (AMR) can the add mesh elements to increase the resolution and reduce the numerical error, $\|u_h-u\|$ in some norm.  For VI problems, however, the simulation goal is often the accurate approximation of the sets \eqref{eq:contactsets}, e.g.~$E_h \approx E_u$ for the inactive set, attainability of which is problem-dependent.
