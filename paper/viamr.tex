...
Our first two methods for adaptive mesh refinement (AMR) for VIs, namely Algorithms \ref{alg:udo} and \ref{alg:vcd} below, are of \emph{tag-and-refine} type.  Since these methods do not coarsen, we compare them to a more-expensive third method, a \emph{metric-based mesh adaptation} approach \cite{Wallworketal2020}, which can hold the mesh complexity approximately constant.

Each method starts from a computed solution to FE VI problem \eqref{eq:fe:vi}; we need only the mesh $\cT_h$, the admissible solution $u_h \in \cK_h \subset \cX_h$, and the obstacle $\psi_h \in \cX_h$.  The vertices of $\cT_h$, which are the degrees of freedom of $\CG_1(\cT_h)$, will be denoted $x_j$.  The following two concepts are fundamental.

\begin{definition} \label{def:nodalactive}
Given $u_h \ge \psi_h$ and a tolerance $\text{tol}>0$, the \emph{(a posteriori) nodal active set indicator} is the function $\nu_h\in\CG_1(\cT_h)$ satisfying
...
However, a primary quantity of interest for obstacle problems is \emph{vicinity to the unknown free boundary}.  To our knowledge there is no precise functional which computes this quantity.  Therefore Algorithms \ref{alg:udo} or \ref{alg:vcd} proceed heuristically to convert an \emph{a posteriori} nodal active set indicator $\nu_h$ (Definition \ref{def:nodalactive}) into an element marking $I_h$ (Definition \ref{def:marking}), with the goal of improving the approximation of the free boundary.  For these two Algorithms, from the marking $I_h$ we then apply \emph{skeleton-based refinement} (SBR) \cite{PlazaCarey2000} to generate a refined mesh with no hanging nodes and good mesh quality.  Note that two implementations of SBR are available to Firedrake applications, namely in the DMPlex component of PETSc \cite{petsc-user-ref}, or from the Netgen library \cite{Betteridgeetal2024}, but only the latter implementation is (currently) capable of 3D refinement.

The use of queries to a PETSc DMPlex object is essential to Algorithm \ref{alg:udo} below, so we sketch some aspects of how domain management DMPlex class supports unstructured meshes \cite{Langeetal2016}, and its query system.  A DMPlex object stores the topology (connectivity of entities) of a $d=1,2,3$ FE mesh, while the geometry (coordinates of vertices) is in an associated PetscSection object.  Every mesh entity, regardless of dimension, is assigned a unique index.  Mesh connectivity can be understood as a stratified directed acyclic graph (DAG) in which a covering (incidence) relationship between mesh entities determines the directed edges.  For example, a triangle within a 2D mesh is covered by its three edges, each of which is covered by its two endpoints (vertices).  Each stratum in the DAG represents a dimensional class of mesh entity.  For example the cells, edges, and vertices in a 2D mesh are at strata index---this is the codimension or ``height''---of 0,1,2 respectively.

Given the index $p$ of a mesh entity, we will use the following DMPlex mesh topology queries \cite{petsc-user-ref}.  $\text{emph{cone}}(p)$ returns all entities which cover $p$.  The dual operation of \emph{cone}, which returns all the entities which $p$ covers, is $\text{\emph{support}}(p)$.  The transitive closure of \emph{cone} is $\text{\emph{closure}}(p)$.  The transitive closure of \emph{support} is $\text{\emph{star}}(p)$.  When applying $\text{\emph{closure}}(p)$ we will only extract the indices of vertices, and when applying $\text{\emph{star}}(p)$ we will only extract the indices of elements.

With this preparation we can finally describe the Unstructured Dilation Operator (UDO) method, which is shown in Algorithm \ref{alg:udo}.  It first computes a nodal active set indicator $\nu_h$ (Definition \ref{def:nodalactive}).  Then it finds the indices of the border elements $K$ such that the $\DG_0(\cT_h)$ degree of freedom $x_K$ gives a value $\nu_h(x_K)$ strictly between 0 and 1, that is, so that at least one of the vertices incident to $K$ is in the active set, and at least one is not.  Then the method alternates \emph{closure} and \emph{star} on these border elements $n$ times, to expand the free boundary by $n$ elements.  It returns an element marking $I_h$ (Definition \ref{def:marking}).
