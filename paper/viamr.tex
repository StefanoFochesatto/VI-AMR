...
The first two new methods for adaptive mesh refinement (AMR) for VIs, in this Section, are of \emph{tag-and-refine} type.  Since these methods do not coarsen, we then compare to a third method, a more-expensive \emph{metric-based mesh adaptation} approach \cite{Wallworketal2020}, which can hold the mesh complexity approximately constant.

Our methods start from a computed solution to FE VI problem \eqref{eq:fe:vi} over a mesh $\cT_h$, namely the admissible solution $u_h \in \cK_h \subset \cX_h$ and the obstacle $\psi_h \in \cX_h$.  The following two concepts are fundamental.

\begin{definition} \label{def:nodalactive}
Given $u_h$, $\psi_h$, and a tolerance $\text{tol}>0$, the \emph{nodal active set indicator} is the function $\nu_j\in\CG_1(\cT_h)$, with values $\{0,1\}$ on the vertices $x_j\in\cT_h$, satisfying
	$$\nu_j = \begin{cases} 1, & u_h(x_j) - \psi_h(x_j) < \text{tol} \\
	                        0, & \text{otherwise}\end{cases}$$
\end{definition}

\begin{definition} \label{def:marking}
An \emph{element marking} (\emph{tagging}) of a mesh $\cT_h$ is a piecewise-constant refinement indicator function $I_K \in \DG_0(\cT_h)$ with values in $\{0,1\}$.  Elements $K \in \cT_h$ such that $I_K=1$ will be refined.
\end{definition}

For PDE problems, an element marking typically targets a quantity of interest, such as energy or $L^2$ norm error, or a local functional \cite{BangerthRannacher2003}, represented by an error indicator $\eta_K$.  (For the interior condition PDE of problem \eqref{eq:vi}, we discussed such indicators in Section \ref{sec:aposteriori}.)  However, a primary quantity of interest for obstacle problems is \emph{vicinity to the unknown free boundary}.  To our knowledge there is no precise functional which computes this quantity.  Therefore our methods proceed heuristically to convert an \emph{a posteriori} nodal active set indicator $\nu_j$ (Definition \ref{def:nodalactive}) into an element marking $I_K$ (Definition \ref{def:marking}), with the goal of improving the approximation of the free boundary.

Our tag-and-refine AMR methods solve FE VI problem \eqref{eq:fe:vi}, then apply one of the two Algorithms below to generate a marking $I_K$, and then apply \emph{skeleton-based refinement} (SBR) \cite{PlazaCarey2000} to generate a refined mesh with no hanging nodes and good mesh quality.  Two implementations of SBR are available in Firedrake applications, namely from the PETSc DMPlex component \cite{petsc-user-ref}, or from the Netgen library \cite{Betteridgeetal2024}; only the latter is (currently) capable of 3D refinement.

FIXME the algorithm uses the DMPlex \emph{closure} and \emph{star} operations \cite{Langeetal2016}; note $x_K$ denotes the $\DG_0$ degree of freedom for element $K$; note $B_K=N(\Gamma_h)^i$ at the start of each pass in the \textbf{for} loop; note $\ell_j$ will denote number of elements incident to a vertex $j$
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		...
		\State Compute the nodal active set indicator $\nu_j \in \CG_1(\cT_h)$ (Definition \ref{def:nodalactive}).
		\State Define the initial border marking $I_K \in \DG_0(\cT_k)$ by $I_K=1$ if $\nu_j(x_K) \in (0,1)$, and zero otherwise.
		\For{$i=0,\dots,n-1$}
		    \State For elements $K$ with $I_K=1$, find adjacent vertices $M_K=\{j_0,\dots,j_d\}$ using \emph{closure}.
		    \State For vertices $j$ in some $M_K$, find adjacent elements $M_j = \{K_0,\dots,K_{\ell_j-1}\}$ using \emph{star}.
		    \State Set $I_K=1$ for all elements $K$ in some $M_j$.
		\EndFor \\
		\Return marking $I_K$
	\end{algorithmic}
\label{alg:udo}
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		...
		\State Approximately solve $s_h - \Div(D \grad s_h) = \nu_j$, i.e.~one step of \eqref{eq:diffusioneqn}, for $s_h\in\CG_1$.
		\State Compute the element marking $I_K \in \DG_0(\cT_h)$ by thresholding the values of $s_h$ at the $\DG_0$ degrees of freedom $x_K$:
		\begin{equation*}
		  I_K = \begin{cases}
