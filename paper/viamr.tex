...
\caption{Meshes computed from the UDO (left), VCD (middle), and AVM (right) methods, on the obstacle problem shown in Figure \ref{fig:ball}, and in the middle of Figure \ref{fig:activesizes}.}
\label{fig:threeballmeshes}
\end{figure}

Example meshes are shown in Figure \ref{fig:threeballmeshes}.  They were generated by 3 levels of refinement using the above three methods, from a uniform mesh.  In each case the refinement targeted both increased resolution near the free boundary and in the inactive set.
