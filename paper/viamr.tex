...
The effectiveness of AMR strategies for VI problems depends upon the measure (area or volume) of the active and inactive sets.  For certain obstacle-type problems, specifically those with the ``blistering property'' defined in Section \ref{sec:vifem}, the elements which have been identified as interior to the active set may require no further computation or refinement.  For such problems, if a solution is desired at high resolution within the active set, this can be computed in post-processing by arbitrary interpolation of the obstacle, noting $\psi$ has already been provided in the data of the problem.
...
\item We provide a parallel, well-documented, and easy-to-use implementation, hosted at \href{https://github.com/StefanoFochesatto/VI-AMR}{\texttt{github.com/StefanoFochesatto/VI-AMR}}, within a widely-distributed FE library framework, namely Firedrake \cite{Langeetal2016}.
\end{enumerate}
