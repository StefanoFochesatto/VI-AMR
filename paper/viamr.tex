...
Two of our new methods for adaptive mesh refinement (AMR) for VIs are of \emph{tag-and-refine} type.  We introduce those algorithms first.  However, since these methods do not coarsen, after that we will compare to a more-expensive metric-based framework \cite{Wallworketal2020} which can hold the mesh complexity approximately constant.

Tagging generally targets quantities of interest, such as energy or $L^2$ norm error, or a local functional \cite{BangerthRannacher2003}, to generate error indicators $\eta_K$ for each element $K$.  For the interior condition (inactive set PDE) of problem \eqref{eq:vi}, we discussed such indicators $\eta_K$ in Section \ref{sec:aposteriori}.  However, a primary quantity of interest for obstacle problems is \emph{vicinity to the unknown free boundary}.  To our knowledge there is no precise functional which computes this quantity.  Therefore our methods proceed heuristically to some degree.

Our tag-and-refine methods start from a computed solution $u_h \in \cK_h$, over mesh $\cT_h$, to FE VI problem \eqref{eq:fe:vi}.  From $u_h$ they generate a piecewise-constant refinement indicator function $I_h \in \DG_0(\cT_h)$ with values in $\{0,1\}$; the elements $K \in \cT_h$ such that $I_h(K)=1$ are the ones which will be refined.  We then apply \emph{skeleton-based refinement} (SBR) \cite{PlazaCarey2000}, which generates meshes with no hanging nodes and good mesh quality.  Two implementations of SBR are available, namely from the PETSc DMPlex component \cite{petsc-user-ref}, or from the Netgen library \cite{Betteridgeetal2024}.  Only the latter is (currently) capable of 3D refinement.  FE VI problem \eqref{eq:fe:vi} is then solved again over the refined mesh, with AMR iterations until a desired resolution is achieved.
...
The initial mesh, marked elements near the free boundary, and refined mesh are shown in Figure \ref{fig:resultAOL}.
\end{example}
