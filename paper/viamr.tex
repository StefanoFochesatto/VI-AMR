...
\end{equation}
The same argument given for \eqref{eq:vi} shows that \eqref{eq:fe:vi} uniquely defines $u_h$.

Because $\cK_h \subset \cX_h \subset \cX$, we may regard \eqref{eq:fe:vi} as a conforming FE method for \eqref{eq:vi}.  However, the meaning of ``conforming'' is subtle for such an obstacle problem.  No specific relationship between $\psi$ and $\psi_h$ is assumed, although we now consider some possibilities.  If $\psi_h$ is an interpolant ($\psi_h = \Pi_h \psi$) or projection of the continuum obstacle $\psi$ then $\cK_h \approx \cK$ in some sense, but precision is needed for later theory.  There are three levels of approximation, with increasing strength:
...
\end{enumerate}
As noted already by Ciarlet, generic situation \emph{i)} will apply if $\psi_h = \Pi_h \psi \in P_1$ for an original obstacle $\psi$ which is not convex over all elements \cite[Figure 5.1.3]{Ciarlet2002}.  Situation \emph{ii)} holds when $\psi_h \ge \psi$, which can be achieved on a given mesh by using a monotone injection operator, i.e.~``$\psi_h = R_h^\oplus \psi$'' if one applies this tool from multilevel VI solvers \cite{BuelerFarrell2024}.  The strong assumption \emph{iii)} is made in \cite{AinsworthOdenLee1993}; it holds if $\psi_h=\psi$, e.g.~if $\psi=0$.

On the other hand, the obstacle $\psi$ and the source function $f$ are data of the problem.  Recall that within a conforming FE approximation of the Poisson equation (PDE) it is assumed that the load-vector integrals are computed exactly, even though quadrature may be required in any implementation.  Similarly here for obstacle problems we should regard the original obstacle as data which has well-defined values, usable as needed to represent the numerical solution in its identified active set.  Following \eqref{eq:classical:sets}, define
\begin{equation}
  I_u^h = \{x \in \Omega \,:\, u_h(x) > \psi_h(x)\}, \quad A_u^h = \Omega \setminus I_u^h, \label{eq:fe:sets}
\end{equation}
the numerical inactive and active sets defined from the solution to \eqref{eq:fe:vi}.  Over elements $K\in\cT_h$ which are fully contained in $A_u^h$ we may regard the solution as equal to $\psi_h$ or to an interpolant of $\psi$ of arbitrarily higher resolution or order without affecting the computational cost of the solution to \eqref{eq:fe:vi}.
FIXME Said another way, we do not need to solve for the obstacle \emph{per se}, as we solve for $u_h$.  It is important to distinguish between the numerical errors in $u_h$ arising from mis-locating the free boundary, thus mis-identifying the active and inactive sets, versus the quality of interpolation or approximation of the original obstacle $\psi$ within a correctly-identified active set (i.e.~according to $u_h$).
