...
% result from examples/sphere.py; -n 1 UDO and default VCD
\begin{figure}[ht]
\centering
...
Our first two new methods for adaptive mesh refinement (AMR) for VIs, namely Algorithms \ref{alg:udo} and \ref{alg:vcd} below, are of \emph{tag-and-refine} type.  Since these methods do not coarsen, we compare them to a more-expensive third method, a \emph{metric-based mesh adaptation} approach \cite{Wallworketal2020}, which can hold the mesh complexity approximately constant.

Each method starts from a computed solution to FE VI problem \eqref{eq:fe:vi} over a mesh $\cT_h$, specifically using the admissible solution $u_h \in \cK_h \subset \cX_h$ and the obstacle $\psi_h \in \cX_h$.  The vertices of $\cT_h$, namely the degrees of freedom of $\CG_1(\cT_h)$, will be denoted $x_j$.  The following two concepts are fundamental.

\begin{definition} \label{def:nodalactive}
Given $u_h$, $\psi_h$, and a tolerance $\text{tol}>0$, the \emph{nodal active set indicator} is the function $\nu_h\in\CG_1(\cT_h)$ satisfying
	$$\nu_h(x_j) = \begin{cases} 1, & u_h(x_j) - \psi_h(x_j) < \text{tol} \\
	                             0, & \text{otherwise}\end{cases}$$
\end{definition}

\begin{definition} \label{def:marking}
An \emph{element marking} (\emph{tagging}) of a mesh $\cT_h$ is a piecewise-constant indicator function $I_h \in \DG_0(\cT_h)$ with values in $\{0,1\}$.  Elements $K \in \cT_h$ such that $I_h(K)=1$ will be refined.
\end{definition}

For PDE problems, an element marking is typically derived from a quantity of interest, such as energy or $L^2$ norm error, or a local functional \cite{BangerthRannacher2003}, and an associated error estimator $\eta_K$.  For the interior condition PDE of problem \eqref{eq:vi}, we gave examples of such estimators in Section \ref{sec:aposteriori}.

However, a primary quantity of interest for obstacle problems is \emph{vicinity to the unknown free boundary}.  To our knowledge there is no precise functional which computes this quantity.  Therefore Algorithms \ref{alg:udo} or \ref{alg:vcd} proceed heuristically to convert an \emph{a posteriori} nodal active set indicator $\nu_h$ (Definition \ref{def:nodalactive}) into an element marking $I_h$ (Definition \ref{def:marking}), with the goal of improving the approximation of the free boundary.  From the marking $I_h$ we then apply \emph{skeleton-based refinement} (SBR) \cite{PlazaCarey2000} to generate a refined mesh with no hanging nodes and good mesh quality.  Note that two implementations of SBR are available to Firedrake applications, namely from the DMPlex unstructured mesh component of PETSc \cite{petsc-user-ref}, or from the Netgen library \cite{Betteridgeetal2024}.  Only the latter implementation is (currently) capable of 3D refinement.

The use of queries to a DMPlex object is essential to Algorithm \ref{alg:udo} below, so we sketch some aspects of how DMPlex supports unstructured meshes.  It is a domain management class \cite{Langeetal2016} which stores the topology (connectivity of entities) and geometry (coordinates of vertices) of an FE mesh.  Note that every mesh entity is assigned a unique index.   Mesh connectivity is then stored as a stratified directed acyclic graph (DAG) in which a ``covering'' relation determines the edges of the DAG.  For example, a triangle within a 2D mesh is covered by its three edges, each of which is covered by its two endpoints (vertices).  Conceptually-speaking, this covering relation is indicate by a down-arrow in the DAG.  Each stratum in the DAG represents a class of mesh entity; for example the cells, edges, and vertices in a 2D mesh have height strata 0,1,2 respectively.  A DMPlex object also has several methods for querying the mesh topology \citep{Langeetal2016}.  For example, if $p$ is the index of a mesh entity then $\text{\texttt{getCone}}(p)$ returns all entities which cover $p$.  The transitive closure of the cone operation is $\text{\texttt{getTransitiveClosure}}(p)$.  The dual operation of the cone, which finds all the entities which $p$ covers, is $\text{\texttt{getSupport}}(p)$, and its transitive closure is the star operation.

FIXME WITH ALL THIS PREP WE CAN FINALLY SHOW UDO Algorithm \ref{alg:udo}
...
FIXME note $x_K$ denotes the $\DG_0$ degree of freedom for element $K$; note $B_K=N(\Gamma_h)^i$ at the start of each pass in the \textbf{for} loop; note $\ell_j$ will denote number of elements incident to a vertex $j$
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		...
		\State Define the initial border marking $I_h \in \DG_0(\cT_k)$ by $I_h(K)=1$ if $\nu_h(x_K) \in (0,1)$, and zero otherwise.
		\For{$i=0,\dots,n-1$}
		    \State For each element $K$ with $I_h(K)=1$, find adjacent vertices $M_K=\{j_0,\dots,j_d\}$ using \emph{closure}.
		    \State For each vertex index $j \in \bigcup M_K$, find adjacent elements $M_j = \{K_0,\dots,K_{\ell_j-1}\}$ using \emph{star}.
		    \State Set $I_h(K)=1$ for all elements $K \in \bigcup M_j$.
		\EndFor \\
		\Return marking $I_h$
	\end{algorithmic}
\label{alg:udo}
...
\begin{algorithm}[ht]
	...
	\begin{algorithmic}[1]
		...
		\State Compute the element marking $I_h \in \DG_0(\cT_h)$ by thresholding the values of $s_h$ at the $\DG_0$ degrees of freedom $x_K$:
		\begin{equation*}
		  I_h(K) = \begin{cases}
			1 & \text{ if } \alpha < s_h(x_K) < \beta \\
			0 & \text{ otherwise}
		  \end{cases}
		\end{equation*}\\
		\Return marking $I_h$
...
FIXME There are various parameters to consider with this technique. 
...
\mbox{\begin{subfigure}[t]{.45\textwidth}
    \input{tikz/vcd-a.tex}
    \caption{Iterate $u_h$(black) and obstacle $\psi_h$ (red).}
  \end{subfigure}
  \quad
  \begin{subfigure}[t]{.45\textwidth}
    \input{tikz/vcd-b.tex}
    \caption{Nodal active set indicator $\nu_h$.}
  \end{subfigure}}

\medskip
\mbox{\begin{subfigure}[b]{.35\textwidth}
    \centering
    \input{tikz/vcd-c.tex}
    \caption{Smoothed $s_h$.}
  \end{subfigure}
  \quad
  \begin{subfigure}[b]{.35\textwidth}
    \centering
    \input{tikz/vcd-d.tex}
    \caption{Threshold values in red.}
  \end{subfigure}}
...
\medskip
  \begin{subfigure}[b]{0.9\textwidth}
    ...
    \caption{Refinement indicator function $I_h$.}
  \end{subfigure}
...
We end this section with basic Python examples showing how the library\footnote{\href{https://github.com/StefanoFochesatto/VI-AMR}{\texttt{github.com/StefanoFochesatto/VI-AMR}}} is applied.

\begin{example} \label{example:AOL}
Consider Example 1 from \cite{AinsworthOdenLee1993}, a classical obstacle problem over a rectangle, with obstacle $\psi=0$ and a known exact solution.  The following complete code, using the Firedrake and VIAMR libraries, generates a coarse mesh and then applies VCD marking and refinement around the free boundary:
\inputminted[linenos, frame=lines]{python}{static/exampleAOL.py}
\medskip
\noindent The initial mesh, marked elements near the free boundary, and refined mesh are shown in Figure \ref{fig:resultAOL}.
\end{example}
