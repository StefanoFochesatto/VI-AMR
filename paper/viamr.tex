...
The obstacle problems addressed by the VIAMR library substantially generalize the above classical continuum problem.  The scope of the library is defined in the following boxes.

\begin{libboxtitle}{VIAMR library scope includes:}
  \begin{itemize}
  \item scalar obstacle problems in which
  \item the constraint set is defined by upper and lower obstacles, and
  \item the residual operator may have any (nonlinear) formula.
  \end{itemize}
\end{libboxtitle}

The continuum VI problem is thus defined by certain data over a bounded, polygonal domain $\Omega\subset \RR^d$.  This includes a Dirichlet datum $g\in C(D)\strut$, over well-behaved $D\subset\partial\Omega$, and two continuous functions $\lb,\ub \in C(\bar\Omega)$, the \emph{box constraints}.  These data must satisfy compatibility conditions:
\begin{libbox}
\begin{equation}
...
\left<F(u),v - u\right> \ge 0 \quad \text{ for all } v \in \cK,
\end{equation}
\end{libbox}
\noindent Here $F:\cK \to \cX'$ is a continuous \emph{residual function}, defined for admissible inputs, or even over all of $\cX$.  Note that $\cX'$ denotes the topological dual space, and $\left<\cdot,\cdot\right>$ the dual pairing.  For example, $\left<F(u),w\right> = \int_\Omega \nabla u \cdot \nabla w - fw$ in classical problem \eqref{eq:classical:vi}.
...
\end{enumerate}
Noting that we are restricting to $P_1$ elements, a mesh can be found which forces situation (i) if $\lb$ is somewhere concave or $\ub$ is somewhere convex.  Situation (ii) can be imposed either by restricting the constraint convexity or by use of monotone injection operators to define $\lbh$, $\ubh$ \cite{BuelerFarrell2024,GraeserKornhuber2009}; see also the proof of Theorem 5.1.2 in \cite{Ciarlet2002}.  The strongest condition (iii) holds if the continuum constraints $\lb$, $\ub$ are themselves affine, e.g.~in the section \ref{sec:results} unilateral examples for which $\lb=0$.
...
\left<F_h(u_h), v_h - u_h\right> \ge 0 \quad \text{ for all } v_h \in \cK_h.
\end{equation}
...
As with the continuum problem, solving \eqref{eq:fe:vi} for $u_h$ simultaneously defines subsets of $\Omega$:
\begin{gather}
\label{eq:fe:sets}
...
FIXME address nodal and elementwise interpretation

FIXME bullet list analogous to (but generalized) that in NSV03
