...
\end{gather}
These sets are accessible \emph{a posteriori} like $u_h$ itself.  The contact sets $\Cbotuh,\Ctopuh$ are finite unions of nodes, closed edges (faces), and closed elements up to dimension $d$.  The inactive set $E_h$ is open.  The free boundary $\Gamma_h$ is a finite union of closed nodes, edges, etc.~up to dimension $d-1$.  (It cannot contain element-interior points because, in the $P_1$ space, if $u_h=\lbh$ at such a point then that entire element is in $\Cbotuh$.)
% FIXME address nodal and elementwise interpretation more?

Now, for the unilateral classical obstacle problem, Nochetto, Siebert, and Veeser \cite{NSV03} outlined properties that \emph{a posteriori} estimators and associated adaptive procedures should exhibit.  Here is their recommended list, adapted to our box-constrained case \eqref{eq:fe:admissible}--\eqref{eq:fe:sets}:
...
\item In the inactive set $E_h$, estimators should be insensitive to the obstacles $\lb,\ub$, and reduce to an expected estimator for the (unconstrained) PDE.
...
\item If the residual measure $\sigma = F(u)-\ell$ happens to be singular with respect to the Lebesgue measure, then we expect strong refinement near the discrete free boundary.  A piecewise linear approximation of $\sigma$ cannot be very accurate in such a case.
\item It is not advisable to preadapt the mesh according to data $\ell,\lb,\ub$ since the contact set is unknown beforehand and its local mesh size should only depend on obstacles but not $\ell$, whereas the opposite situation occurs in the non-contact set.
\end{enumerate}

The above properties are fulfilled to varying degrees by the methods of this paper, with maximum success in the easier cases, such as classical obstacle problem with affine obstacles.  FIXME sometimes can't achieve because of missing rigor FIXME we agree (a),(b).  we implemented \cite{NSV03,NSV05} in library.  we have concerns about (c),(d),(e).  we add (f): refinement in active can be wasted, esp re blistering property problems Appendix \ref{app:blistering} FIXME which are actually \emph{goals} for the methods in our library?
