...
\begin{example}  Consider the ``spiral'' example from \cite[subsection 7.1.1]{GraeserKornhuber2009}, on the square $\Omega=(-1,1)^2$, shown already in Figure \ref{fig:activesizes} (left).  The active set is small in area while the free boundary is long.  A result from applying the UDO$+$BR approach is shown in Figure \ref{fig:spiralmesh}.  (Results from VCD$+$BR and AVM approaches are very similar and not shown.)  At this resolution the free boundary appears to be well-resolved, but for this example we see no performance advantage from avoidance of refinement in the small active set. \end{example}
...
\begin{example}  The example shown in Figure \ref{fig:activesizes} (right), by contrast, has a large active set. FIXME $\psi = 0$, and nonegative source term $f$ with homogeneous Dirichlet conditions  \end{example}

\begin{figure}[ht]
\centering
\mbox{
\includegraphics[height=60mm]{meshfigs/blisters.png} \qquad
\includegraphics[height=60mm]{meshfigs/blisterszoomed.png}
}
\caption{FIXME}
