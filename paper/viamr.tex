...
\end{itemize}
That is, the continuum VI problem expected by the library, stated by \eqref{eq:admissible} and \eqref{eq:vi} below, is defined by a polygonal domain $\Omega\subset \RR^d$, a function space $\cX \subset L^2(\Omega)\strut$, boundary data $D\subset\partial\Omega$ and $g\in C(\bar D)\strut$, and two continuous functions $\lb,\ub \in \cX \cap C(\bar\Omega)$, the \emph{box constraints}.  Note that the library allows either constraint to have Python value \texttt{None}, allowing unilateral problems.  Also note that fixed Neumann boundary conditions, if any, are regarded as part of the operator definition below; they do not affect the continuum admissible set.

The closed and convex admissible subset $\cK \subset \cX$ is then defined as follows:
\begin{equation} \label{eq:admissible}
\cK = \{u \in \cX \,:\, \lb \le u \le \ub \text{ and } u|_{D} = g\}.
\end{equation}
This set is non-empty because we also assume the compatibility conditions that $\lb < \ub$ on $\Omega$ and $\lb \le g \le \ub$ on $D$.  Then, for a residual function $F:\cK \to \cX'$, the general VI problem covered by the library finds $u\in \cK$ so that
\begin{equation} \label{eq:vi}
\left<F(u),v - u\right> \ge 0 \quad \text{ for all } v \in \cK,
\end{equation}
where $\left<\cdot,\cdot\right>$ is the dual pairing.  For example, $\left<F(u),w\right> = \int_\Omega \nabla u \cdot \nabla w - fw$ in the classical problem.

Let us say a bit more about continuum problem \eqref{eq:vi} before addressing its FE solution.  When $\cX = W^{1,p}(\Omega)$, for $p>1$, then \eqref{eq:vi} is well-posed under certain conditions from the theory of monotone operators.  Specifically, if $F$ is strictly monotone, coercive, and continuous then a unique solution exists \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  These conditions apply to the classical problem \eqref{eq:classical:vi} and its $p$-Laplacian extensions \cite{LiuBarrett1994}.  They also apply to the advection-diffusion problem in section \ref{sec:results}, and, with appropriate adjustments \cite{JouvetBueler2012}, to flat-topography cases of the highly-nonlinear glacier geometry problem in section \ref{sec:app}.  However, as for any numerical solver and/or meshing libraries, it is the user's responsibility to apply the library to a well-posed problem.

Returning to the idea of continuum complementarity, as illustrated by \eqref{eq:classical:ncp} for the classical problem, the solution residual $F(u)\in \cX'$ is generally highly-irregular but it has certain sign properties when $\cK$ is defined by box constraints as here.  Write
\begin{equation}
\label{eq:contactsets}
\Cbotu = \{x\,:\,u(x) = \lb(x)\} \quad \text{ and } \quad \Ctopu = \{x\,:\,u(x) = \ub(x)\}
\end{equation}
for the \emph{lower-} and \emph{upper-contact} (active) subsets of $\Omega$, with $I_u = \Omega \setminus (\Cbotu \cup \Ctopu)$ the open inactive set.  Now $F(u) \ge 0$ on the open set $\Omega \setminus \Ctopu$, and likewise $F(u) \le 0$ on $\Omega \setminus \Cbotu$, in the distributional sense.  That is,\footnote{Prove this by taking a smooth and locally-supported test function $\varphi\in \cX$, with $\varphi\ge 0$ compactly-supported in $\Omega \setminus \Ctopu$, and substitute $v=u+\eps \varphi$ into \eqref{eq:vi}.  For sufficiently small $\eps>0$, such that $v\in\cK$, then $\left<F(u),v - u\right> = \eps \left<F(u),\varphi\right> \ge 0$.  The second equality is similar.} $\left<F(u),\varphi\right> \ge 0$ for every $\varphi \ge 0$ compactly supported in $\Omega \setminus \Ctopu$, and etc.  As a distribution, $F(u)$ is a signed measure \cite{EvansGariepy2015} which admits a decomposition $F(u) = \mu_{\bot,u} - \mu^{\top}_u$ into nonnegative Radon measures supported on $\Cbotu$ and $\Ctopu$ respectively \cite[Theorem II.6.9]{KinderlehrerStampacchia1980}.  We therefore have a dual property to the box constraints \eqref{eq:admissible} defining $\cK$:
\begin{equation}
\label{eq:Fsign}
  F(u) \ge 0 \text{ on } \Cbotu \quad \text{and} \quad F(u) \le 0 \text{ on } \Ctopu,
\end{equation}
with $F(u)=0$ on their complement $I_u$.

The VIAMR library assumes a piecewise-linear ($P_1$) or bilinear ($Q_1$) FE method for problem \eqref{eq:vi}.  Restricting to the former for concreteness, let $\cT_h$ be a shape-regular mesh triangulation of $\Omega$ \cite{ElmanSilvesterWathen2014}, let $\cX_h \subset \cX \cap C(\bar\Omega)$ be the finite-dimensional $P_1$ FE subspace.  Denote the interpolation operator by $I_h : C(\bar\Omega) \to \cX_h$.  

FIXME MAKE BOX FROM HERE Let $\psi_h \in \cX_h$ be the FE obstacle, which satisfies the compatibility requirement $\psi_h \le g_h$ along $\partial\Omega$.  Define the (nonempty) FE admissible set
...
\begin{equation} \label{eq:admissibleOLD}
...
The strong-form interior condition for \eqref{eq:sia:vi} is a conservation equation, namely $\Div \bq = \tilde a$ over $I_u$, where the flux is given by
\begin{equation}
