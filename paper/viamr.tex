...
A small modification of \eqref{example:oned} shows an important aspect of obstacle problems: the sets $A_u,I_u,\Gamma_u$ are \emph{not} continuous functions of the data.

\begin{example} \label{example:notcontinuous} FIXME In Example \ref{example:oned} set $g=-0.5$ so that $g=\psi$ on $\partial\Omega$.  Since $f=0$ we see that $u=\psi$ is the solution and $A_u=\Omega$.  For $\eps>0$, now let $f_\eps(x) = $
\end{example}
