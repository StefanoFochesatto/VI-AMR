...
\begin{keywords}
variational inequality, free boundary, obstacle problem, finite element, adaptive mesh refinement
\end{keywords}
...
\begin{example} \label{example:notcontinuous}  Suppose $\Omega = (-1,1)$, $\psi(x)=1 - x^2$, and $g(x)=0$, but let $f$ be constant: $f(x)=\alpha$.  The exact solution $u$ of \eqref{eq:classical:vi} is easily calculated: $u(x)=\psi(x)$ if $\alpha\le 2$ and $u(x)= 0.5 \alpha \psi(x)>\psi(x)$ if $\alpha>2$.  Thus for $\alpha \ge 2$ we have $A_u=\Omega$ and $I_u=\emptyset$, while $A_u=\emptyset$ and $I_u=\Omega$ if $\alpha<2$.  This example shows that the sets \eqref{eq:classical:sets} are not continuous functions of the data $f$ of the problem.  It can be easily modified to show that the sets are not continuous functions of $\psi$ either.
\end{example}
...
\begin{example}  \label{example:gradrecovery}  Suppose $\cX_h=\CG_k$ is the continuous, piecewise-polynomial FE space of degree $k$, and consider $u_h\in\cX_h$.  Generally $\grad u_h$ is discontinuous but well-defined on each element.  If $\cY_h=\DG_{k-1}$ denotes the (scalar) discontinuous space with polynomial degree $k-1$ then $\grad u_h \in \cY_h^d$.  Suppose there is a linear operator $G : \cX_h \to \cX_h^d$, called \emph{gradient recovery} (GR) \cite[Chapter 4]{AinsworthOden2000}, so that $G(u_h)\approx \grad u$ in some sense, assuming of course that $u_h\approx u$ in $\cX$.  Then, over an element $K \in\cT_h$, the error indicator $\eta_K\ge 0$ for this GR method is
...
The Unstructured Dilation Operator (UDO) method, Algorithm \ref{alg:udo}, first computes a nodal active set indicator $\nu_h$ from $u_h$.  Denoting the degrees of freedom in $\DG_0(\cT_h)$ by $x_k$, it finds the set of indices $k$ of elements such that $0<\nu_h(x_k)<1$.  If $u_h \in \CG_1$ then this condition holds when the element is incident to both active and inactive vertices.  Then the method alternates closure and star on the element set $n$ times, expanding the marking by $n$ element layers, to generate an element marking $\oneh$.  The main motivation here is that if the approximation $u_h\approx u$ is good for a non-degenerate VI problem \eqref{eq:vi} then the free boundary $\Gamma_u$ should pass through, or close to, the elements indicated \emph{a posteriori} by $\oneh$.  In Sections \ref{sec:results} and \ref{sec:app} we will only consider $n=1,2$, as this much expansion seems to suffice for accurate representation of the free boundary after a few refinements.
...
It turns out to be helpful, e.g.~when solving the highly-nonlinear obstacle problem in Section \ref{sec:app}, to add a minimum element diameter $\hmin$ to Algorithms \ref{alg:udo} and \ref{alg:vcd}.  After running these Algorithms, our implementation un-sets the indicator for element $K$, i.e.~to $\oneh|_K=0$, for any element with $h_K < \hmin$.

From a marking $\oneh$, computed by either Algorithm, we then apply \emph{skeleton-based refinement} (SBR) \cite{PlazaCarey2000} to generate a refined mesh with good mesh quality.  Elements with $\oneh=1$ are refined, and SBR refines other elements to avoid hanging nodes.  Two SBR implementations of available from Firedrake, namely in PETSc DMPlex \cite{petsc-user-ref} and in the Netgen library \cite{Betteridgeetal2024}.  Only the latter is currently capable of 3D refinement.
...
We start with a revised look at how numerical errors should be measured for obstacle problems.  Then we show convergence and performance results, from our new AMR methods, on classical obstacle problems \eqref{eq:classical:vi}.  Section \ref{sec:app} will show results for a more-challenging obstacle problem.
...
\caption{Left: $L^2$ norm errors for the UDO$+$BR method, versus uniform refinement.  (Results from VCD$+$BR are virtually identical.)  Right: The same norm errors for the AVM method.}
\label{fig:ballconv}
\end{figure}
...
\begin{figure}[ht]
\centering
\includegraphics[height=62mm]{static/spiralmesh.png}
...
\end{equation}
satisfying $\phi(0,0)=0$ and $\Phi(0,0)=0$.  Then for $u$ solving \eqref{eq:vi}, $f\le 0$ a.e.~in $A_u$.
\end{lemma}
