...
To this highly-nonlinear problem we apply a $u_h\in\CG_1(\cT_h)$ FE method, the UDO and VCD tag-and-refine methods, and a gradient-recovery error indicator in the inactive set.  (See Example \ref{example:gradrecovery}; note that the linear derivation of the BR error indicator in Example \ref{example:br} does not apply.)

; Figure \ref{fig:glacier}
...
\caption{Left:  The glaciated region (grayscale thickness in meters) with a mesh (red) found by applying UDO and a gradient-recovery indicator in the inactive set.  Right:  Detail of glacier margin, at higher refinement.  The surface elevation gradient (grayscale magnitude) changes by $O(1)$ at the margin.}
\label{fig:glacier}
\end{figure}
