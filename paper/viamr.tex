...
In a unilateral obstacle problem the solution is constrained to be above (or below) a certain function, the obstacle, while solving a partial differential equation (PDE) whereever it is not in contact.  These problems are posed as variational inequalities (VIs) \cite{KinderlehrerStampacchia1980}, which are weak-form inequalities extending PDE models from fixed-location boundary conditions to the simultaneous determination of both the solution and the location of free boundaries \cite{Friedman1982}, as an intrinsic part of the solution process.  These problems appear in many scientific, engineering, and economic applications  \cite{FacchineiPang2003,KinderlehrerStampacchia1980,Rodrigues1987} in which the solution is subject to inequality bounds.

The finite element (FE) method was adapted to the solution of VI problems relatively early, for example see the \emph{a priori} estimate in \cite{Falk1974} or the well-known 1978 textbook on FE by Ciarlet \cite{Ciarlet2002}.  The literature of FE solutions to VIs is now large, and we address some this literature, addressing mesh refinement in particular, later in this Introduction.  However, VI problems continue to represent challenging problems for solvers and meshing methods, and substantial developments in concepts and solvers have occurred in recent years, including \cite{BuelerFarrell2024,KeithSurowiec2024,KirbyShapero2024,PapadopoulosHintermuller2026} for example.

This paper introduces the VIAMR\footnote{Download or clone the library from \href{https://github.com/StefanoFochesatto/viamr}{\texttt{github.com/StefanoFochesatto/viamr}}.} Python library for adaptive mesh refinement and mesh adaptation on bilateral, scalar obstacle problems, built on the Firedrake \cite{Rathgeberetal2016} FE library.  To describe this new library we recall some example obstacle problems, state the general case which the library addresses, and then review some of the literature of \emph{a posteriori} estimators for FE solutions.

The classical, unilateral Laplacian obstacle problem \cite{Evans2010,KinderlehrerStampacchia1980} is the canonical case.  This model finds the equilibrium vertical displacement $u$ of an elastic membrane, restricted to small deflections, over a domain $\Omega \subset \RR^d$, typically with $d \in \{1,2,3\}$.  The membrane, attached with displacement $g$ at the fixed boundary $\partial\Omega$, is subjected to an applied force $f$, but it is also constrained to be above a given obstacle $\psi$.  Thus the strong formulation is a complementarity problem over $\Omega$:
...
From a solution of \eqref{eq:classical:ncp} we may identify certain subsets of $\Omega$.  The Poisson equation $-\nabla^2 u = f$ holds on the \emph{inactive set} $I_u=\{x \in \Omega \,:\, u(x) > \psi(x)\}$, while $u=\psi$ on the \emph{active} or \emph{contact} set $C_u = \Omega \setminus I_u$, and $\Gamma_u = \Omega \cap \partial I_u$ is called the \emph{free boundary}.  In applications the determination of these sets may be the primary purpose of the numerical solution, or of comparable importance to determining $u$ itself.

An example is shown in Figure \ref{fig:ball}, where the obstacle $\psi$ is an upper hemisphere, the active set $C_u$ (white mesh) is a disc, and the free boundary $\Gamma_u$ is a circle.  (The obstacle is shown as a sphere for merely aesthetic reasons.)  The Poisson equation ``interior condition'' \cite{KinderlehrerStampacchia1980} holds on the black mesh.  Note that both Dirichlet ($u=\psi$) and Neumann ($\partial (u-\psi)/\partial n = 0$) conditions apply along the free boundary $\Gamma_u$ \cite{Friedman1982}.
...
Another physical interpretation of this classical problem is that $u$ models the water pressure in a porous dam \cite{AinsworthOdenLee1993}; the pressure cannot go below zero ($u\ge\psi=0$), and the goal is to determine which portion of the dam is saturated.  In yet another physical interpretation, wherein the unilateral constraint is an upper bound ($u\le \psi$), the solution $u$ is the displacement in elastic/plastic torsion of a beam, and the goal is to determine which portion has failed plastically \cite{Kornhuber1994}.

Mathematical precision suggests that we identify a function space and a weak formulation for problem \eqref{eq:classical:ncp}, that is, a VI over a Sobolev space.  Let $\Omega \subset \RR^d$ be the domain, let $\cX=H^1(\Omega)$ \cite{Evans2010}, and suppose $\psi \in \cX \cap C(\bar\Omega)$.  Let $g:\partial \Omega\to \RR$ be continuous, with $g\ge\psi|_{\partial \Omega}$, and define the closed and convex \emph{admissible subset}:
\begin{equation} \label{eq:classical:admissible}
\cK = \{u \in \cX \,:\, u \ge \psi \text{ and } u|_{\partial \Omega} = g\} \subset \cX.
\end{equation}
For $f\in L^2(\Omega)$, the weak re-formulation of \eqref{eq:classical:ncp} finds $u\in \cK$ so that
\begin{equation} \label{eq:classical:vi}
\int_\Omega \nabla u \cdot \nabla(v - u) \ge \int_\Omega f(v - u) \quad \text{ for all } v \in \cK.
\end{equation}
Ignoring regularity and speaking intuitively, \eqref{eq:classical:vi} says $\left<-\nabla^2 u - f, v-u\right>\ge 0$, thus that a vector $v-u$ pointing arbitrarily from the solution $u$ into $K$ is at most a right angle away from the residual $F(u) = -\nabla^2 u - f$.  In particular, if $u$ is in the interior $\cK^\circ$ then the residual must be zero, and then $u$ will solve the Poisson equation, and furthermore $I_u=\Omega, A_u=\emptyset, \Gamma_u = \emptyset$.

The obstacle problems addressed by the VIAMR library include the above classical continuum problem, but generally extended in two ways: the residual operator can have any (nonlinear) formula and the constraint set may be defined by upper and lower obstacles (\emph{box constraints}).  That is, the continuum VI problem is defined by a polygonal domain $\Omega\subset \RR^d$, a function space $\cX \subset L^2(\Omega)\strut$, boundary data $D\subset\partial\Omega$, $g\in C(\partial\Omega)\strut$, and two continuous functions $\lb,\ub \in C(\bar\Omega)\cap \cX$.  The closed and convex admissible subset $\cK \subset \cX$ is defined as follows:
\begin{equation} \label{eq:admissible}
\cK = \{u \in \cX \,:\, \lb \le u \le \ub \text{ and } u|_{D} = g\}.
\end{equation}
This set is non-empty because we also assume the compatibility conditions that $\lb < \ub$ on $\Omega$ and $\lb \le g \le \ub$ on $D$.

For a residual function $F:\cK \to \cX'$, the general VI problem covered by the library finds $u\in \cK$ so that
...
Let us say a bit more about continuum problem \eqref{eq:vi} before addressing its FE solution.  When $\cX = W^{1,p}(\Omega)$ for $p>1$ (the Sobolev space of measurable functions with $p$th-integrable weak gradients \cite{Evans2010}) then \eqref{eq:vi} is well-posed under certain conditions from the theory of monotone operators.  Specifically, if $F$ is strictly monotone, coercive, and continuous then a unique solution exists \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  These conditions apply to the classical problem \eqref{eq:classical:vi} and its $p$-Laplacian extensions \cite{LiuBarrett1994}.  They also apply to the advection-diffusion problem in section \ref{sec:results}, and, with appropriate adjustments \cite{JouvetBueler2012}, to flat-topography cases of the highly-nonlinear glacier geometry problem in section \ref{sec:app}.
...
Now the VIAMR library assumes a piecewise-linear ($P_1$) or piecewise-bilinear ($Q_1$) FE solution of problem \eqref{eq:vi}.  Restricting to the former for concreteness, let $\cT_h$ be a shape-regular mesh triangulation of $\Omega$ \cite{ElmanSilvesterWathen2014}, let $\cX_h \subset \cX \cap C(\bar\Omega)$ be the finite-dimensional $P_1$ FE subspace, and denote the interpolation operator by $I_h : C(\bar\Omega) \to \cX_h$.  FIXME MAKE BOX FROM HERE Let $\psi_h \in \cX_h$ be the FE obstacle, which satisfies the compatibility requirement $\psi_h \le g_h$ along $\partial\Omega$.  Define the (nonempty) FE admissible set
