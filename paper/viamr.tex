...
\end{equation}
Here the approximating operator $F_h:\cK_h\to\cX'$ could be defined by a UFL expression for $F$, but generally quadrature and/or regularization is involved in its construction.

If monotonicity and coercivity arguments show well-posedness for \eqref{eq:vi} then the same arguments may show it for \eqref{eq:fe:vi}.  Falk \cite{Falk1974} proved an \emph{a priori} estimate comparing the solutions of \eqref{eq:vi} and \eqref{eq:fe:vi} in the linear operator case, thereby extending Cea's lemma to VIs, and a further extension to nonlinear coercive operators is in \cite[Theorem 5.1]{Bueler2025}.\footnote{This extension adds a term $\left<F(u_h)-F_h(u_h),u_h\right>$ to a Falk-type bound.}

As with the continuum problem, solving \eqref{eq:fe:vi} for $u_h$ simultaneously defines discrete (FE) subsets of $\Omega$:
\begin{gather}
\label{eq:fe:sets}
...
\end{gather}
The contact sets $\Cbotuh,\Ctopuh$ are finite unions of nodes, closed edges (faces), and closed elements up to dimension $d$.  Similarly, the free boundary $\Gamma_h$ is a finite union of closed nodes, edges, etc.~up to dimension $d-1$.  (It cannot contain element-interior points because, in the $P_1$ space, if $u_h=\lbh$ at such a point then that entire element is in $\Cbotuh$.)  The inactive set $E_h$ is open, and if $E_h$ contains $x \in \mathring T$, a point in the interior of an element $T\in E_h$, then it contains the entire element interior.  In any case, and like $u_h$ itself, these sets are accessible \emph{a posteriori}.

The methods of the VIAMR library, summarized at the end of this Introduction, use \emph{a posteriori} information to estimate the errors made in certain elements, or to (heuristically) identify elements near free boundaries where refinement would reduce location errors in the sets in \eqref{eq:fe:sets}.  Of course, adaptive mesh refinement (AMR) uses such \emph{a posteriori} information to strategically add mesh elements to increase the resolution and reduce the numerical error, usually measured by norm differences.  For VI problems, however, the simulation goal is often the accurate approximation of the sets, attainability of which is problem-dependent.

For the unilateral classical obstacle problem, Nochetto, Siebert, and Veeser (NSV) \cite{NSV03} outlined properties that \emph{a posteriori} estimators and associated adaptive procedures should exhibit.  Here is their recommended list, for the box-constrained case \eqref{eq:fe:admissible}--\eqref{eq:fe:sets}:
