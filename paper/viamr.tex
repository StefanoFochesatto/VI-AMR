...
\caption{Solution $u$, as wireframe, to a classical (hemi)spherical obstacle problem over $\Omega \subset \RR^2$.}
\label{fig:ball}
\end{figure}
...
As with the continuum problem, solving \eqref{eq:fe:vi} for $u_h$ determines discrete (FE) subsets of $\Omega$:
\begin{gather}
\label{eq:fe:sets}
...
\end{gather}
The contact sets $\Cbotuh,\Ctopuh$ are finite unions of closed entities (nodes, edges, cells) up to dimension $d$.  Similarly, the free boundary $\Gamma_h$ is a finite union of closed entities up to dimension $d-1$.  (It cannot contain cell-interior points because, in the $P_1$ space, if for example $u_h\in\cK_h$ and $u_h=\lbh$ at an cell-interior point then that entire cell is in $\Cbotuh$.)  The inactive set $E_h$ is open, and if $E_h$ contains $x \in \mathring T$, a point in the interior of a cell $T\in E_h$, then it contains the entire cell interior.  In any case, and like $u_h$ itself, these sets are accessible \emph{a posteriori}.

The methods of the VIAMR library, documented in Section \ref{sec:viamr}, use such \emph{a posteriori} information to estimate the errors made in certain cells, or to (heuristically) identify cells near free boundaries where refinement should reduce location errors in the sets in \eqref{eq:fe:sets}.  Adaptive mesh refinement (AMR) can the add mesh cells to increase the resolution and reduce the numerical error, $\|u_h-u\|$ in some norm.  For VI problems, however, the simulation goal is often the accurate approximation of the sets \eqref{eq:contactsets}, e.g.~$E_h \approx E_u$ for the inactive set, attainability of which is problem-dependent.

For the unilateral classical obstacle problem, Nochetto, Siebert, and Veeser (NSV) \cite{NSV03} proposed properties that \emph{a posteriori} estimators and associated adaptive procedures should exhibit.  Here is their list, for the box-constrained case \eqref{eq:fe:admissible}--\eqref{eq:fe:sets}:
...
\item In the inactive set $E_h$, estimators should be insensitive to the continuum obstacles $\lb,\ub$, and reduce to an estimator for the (unconstrained) PDE.
\item Estimators should detect where the discrete solution is non-conforming, with respect to the continuum obstacles ($u_h\notin \cK$), and refine accordingly.
...
\item It is not advisable to preadapt the mesh according to the data $\ell,\lb,\ub$ since the contact set is unknown beforehand.  In the contact set the local mesh size should only depend on the obstacles $\lb,\ub$, but not $\ell$, whereas the opposite situation occurs in the non-contact set.
\end{enumerate}

For the classical obstacle problem \eqref{eq:classical:vi}, the examples of section \ref{sec:results} show that the above properties are fulfilled by all of the major methods of this paper.  Specifically, the library includes implementations of the NSV estimators \cite{NSV03,NSV05} which, though restricted to the classical case, are proven to have versions of these desirable properties.  In related work \cite{Bueler2026} we have extended the NSV method in \cite{NSV03} to box constraints, which nontrivially changes the internal cell classification in the estimator.  In any case, the rigorous NSV methods are benchmarks for the other, more flexible but less rigorous, methods in the library; see section \ref{sec:results}.

FIXME we agree (a),(b) but we have concerns about (c),(d),(e).  we add (f): refinement in active can be wasted, esp re blistering property problems Appendix \ref{app:blistering}

FIXME actually \emph{goals} for the methods in our library? because our scope includes degenerating-diffusivity operators, essentially (a), (b), (d), (f), with (c), (e) being additional goals for uniformly-elliptic operators.  but these are imperfectly achieved because, to our understanding, new rigorous bounds would be needed for each new class of nonlinear operator

FIXME although we implement methods which exploit the nice structure of the classical Laplacian case, we also implement partly-heuristic methods which perform at least equally well on the classical problem and yet apply naturally to much more realistic cases
