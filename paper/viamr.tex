...
\usepackage{amsmath,amsfonts,amssymb,bbold,stmaryrd,bm}
...
The obstacle problems addressed by the VIAMR library include the above classical continuum problem, but generalized in important directions.  The scope is defined in the following boxes.

\begin{libboxtitle}{VIAMR library scope:}
  \begin{itemize}
  \item scalar obstacle problems defined by VIs, in which
  \item the residual operator can have any (nonlinear) formula, and
  \item the constraint set is defined by upper and lower obstacles.
  \end{itemize}
\end{libboxtitle}

The continuum VI problem expected by the library is defined by certain data, defined over a polygonal domain $\Omega\subset \RR^d$, including a Dirichlet function $g\in C(\bar D)\strut$ over well-behaved $D\subset\partial\Omega$, and two continuous functions $\lb,\ub \in C(\bar\Omega)$, the \emph{box constraints}.  These data must satisfy compatibility conditions:
\begin{libbox}
\begin{equation}
\lb < \ub \text{ on } \Omega \quad \text{and} \quad \lb \le g \le \ub \text{ on } D.\label{eq:compat}
\end{equation}
\end{libbox}
\noindent (Fixed Neumann boundary conditions, if any, are regarded as part of the operator $F$ used in \eqref{eq:vi} below.)  The data functions may be already be in FE spaces, or they may be Unified Form Language (UFL) symbolic expressions \cite{Alnaesetal2014}, but we assume continuity to permit nodal values as needed.  Note that the library enables unilateral constraints by setting either $\lb$ or $\ub$ to Python value \texttt{None}.

Let $\cX$ be a Banach space of scalar functions over $\Omega$, with $C^1(\bar\Omega) \subset \cX \subset L^2(\Omega)$.  The closed and convex admissible subset $\cK \subset \cX$ is defined as:
\begin{libbox}
\begin{equation} \label{eq:admissible}
\cK = \{v \in \cX \,:\, \lb \le v \le \ub \text{ and } v|_{D} = g\}.
\end{equation}
\end{libbox}
\noindent (In this continuum set-up, (in)equalities are almost everywhere.)  The compatibility conditions imply that $\cK$ is nonempty.

The general weak-form problem covered by the library is to find $u\in \cK$ satisfying a variational inequality (VI) of the following form.  Let $F:\cK \to \cX'$ be a continuous residual function, at least defined for admissible inputs, with $\cX'$ denoting the topological dual space.  The VI problem is
\begin{libbox}
\begin{equation} \label{eq:vi}
\left<F(u),v - u\right> \ge 0 \quad \text{ for all } v \in \cK,
\end{equation}
\end{libbox}
\noindent where $\left<\cdot,\cdot\right>$ is the dual pairing.  For example, $\left<F(u),w\right> = \int_\Omega \nabla u \cdot \nabla w - fw$ in classical problem \eqref{eq:classical:vi}.

Let us say a bit more about continuum VI \eqref{eq:vi} before addressing its FE solution.  If $\cX = W^{1,p}(\Omega)$ then \eqref{eq:vi} is well-posed for $F$ satisfying certain conditions from the theory of monotone operators.  Specifically, if $F$ is strictly monotone, coercive, and continuous then it has a unique solution \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  These conditions apply to the classical problem \eqref{eq:classical:vi} and its $p$-Laplacian extensions \cite{LiuBarrett1994}.  They also apply to the advection-diffusion problem in section \ref{sec:results}, and, with appropriate adjustments \cite{JouvetBueler2012}, to flat-topography cases of the highly-nonlinear glacier geometry problem in section \ref{sec:app}.  However, as for any numerical solver/meshing library, it is the user's responsibility to address a well-posed problem.

A complementarity concept, illustrated by \eqref{eq:classical:ncp} for the classical problem, also applies to \eqref{eq:vi}.  That is, the solution residual $F(u)\in \cX'$ may be highly-irregular, but it has certain sign properties because $\cK$ is defined by box constraints \eqref{eq:admissible}.  Write
\begin{gather}
...
\end{gather}
for the \emph{lower-} and \emph{upper-contact} (active) subsets, the \emph{inactive} set, and the \emph{free boundary}, respectively.  The contact sets are disjoint by the compatibility conditions \eqref{eq:compat}.  The free boundary generally has a subset from lower contact and a disjoint subset from upper contact.  Figure \ref{fig:contactsets} illustrates one possibility.
...
\end{equation}
with $F(u)=0$ on their complement $E_u$.  In fact, as a distribution, $F(u)$ is a signed measure \cite{EvansGariepy2015} which admits a decomposition $F(u) = \mu_{\bot,u} - \mu^{\top}_u$ into nonnegative Radon measures supported on $\Cbotu$ and $\Ctopu$ respectively, e.g.~\cite[Theorem II.6.9]{KinderlehrerStampacchia1980}.

Now, the VIAMR library is designed for piecewise-linear ($P_1$) or bilinear ($Q_1$) FE methods for problem \eqref{eq:vi}.  Restricting to the former for concreteness, let $\cT_h$ be a shape-regular mesh triangulation of $\Omega$ \cite{ElmanSilvesterWathen2014}, and let $\cX_h \subset \cX \cap C(\bar\Omega)$ be the finite-dimensional $P_1$ FE subspace.  Denote the interpolation operator by $I_h : C(\bar\Omega) \to \cX_h$, and define $g_h=I_h g$, $\lbh=I_h\lb$, and $\ubh=I_h\ub$.  By the nodal monotonicity property of $P_1$ and $Q_1$ elements \cite[for example]{BuelerFarrell2024}, the same compatibility conditions hold: $\lbh < \ubh$ on $\Omega$ and $\lbh \le g_h \le \ubh$ on $D$.  Therefore define the (nonempty) discrete admissible set as before:
...
\end{equation}
The FE residual function $F_h:\cK_h\to\cX'$ could be defined by the UFL expression for $F$ itself, but generally there are approximations like quadrature or regularization in its construction.\footnote{An \emph{a priori} estimate comparing the solutions of \eqref{eq:vi} and \eqref{eq:fe:vi} is found in \cite[Theorem 5.1]{Bueler2025}.  This extends the classical Falk bound \cite{Falk1974} for VIs, applicable to bilinear forms, specifically by adding a term $\left<F(u_h)-F_h(u_h),u_h\right>$.  In turn, Falk's bound itself extends Cea's lemma \cite{Ciarlet2002}.}  The same monotonicity and coercivity argument which shows well-posedness for \eqref{eq:vi} may well show it for \eqref{eq:fe:vi}.

Similar to the continuum problem, for a solution $u_h$ to \eqref{eq:fe:vi}, we define contact and inactive sets:
\begin{gather}
\label{eq:fe:sets}
\Cbotuh = \{x\,:\,u_h(x) = \lbh(x)\}, \quad \Ctopuh = \{x\,:\,u_h(x) = \ubh(x)\}, \\
E_h = \Omega \setminus (\Cbotuh \cup \Ctopuh). \notag
\end{gather}
These sets, defined \emph{a posteriori} from solving \eqref{eq:fe:vi}, FIXME address nodal and elementwise interpretation

Because $\cK_h \subset \cX_h \subset \cX$, we might regard \eqref{eq:fe:vi} as a conforming FE method for \eqref{eq:vi}.  This is, however, subtle for an obstacle problem, as it depends on the pointwise relationship between the constraints and their interpolants \cite{Ciarlet2002}.  For example, if $\lbh=I_h \lb$, $\ubh=I_h \ub$ are the interpolants then $\cK_h \approx \cK$ in some sense, but generally there is no containment.  One might distinguish three levels of increasingly conformity:
\renewcommand{\labelenumi}{(\roman{enumi})}
\begin{enumerate}
\item $\cK_h \not \subset \cK$,
\item $\cK_h \subset \cK$, and
\item $\cK_h = \cK \cap \cX_h$.
\end{enumerate}
Situation (i) generally occurs when $\lb$ is concave or $\ub$ is convex.  Situation (ii) can be imposed either by a convex/concave assumption, or it can be imposed by use of monotone injection operators to define the discrete constraints \cite{BuelerFarrell2024,GraeserKornhuber2009}; see also the proof of Theorem 5.1.2 in \cite{Ciarlet2002}.  The strongest condition (iii) holds if the constraints are themselves affine, for example, e.g.~including the $\lb=0$ unilateral examples in section \ref{sec:results}.
...
However, as with elliptic PDE problems generally, 
