...
We start this Section with the VI formulation of unilateral obstacle problems in Banach spaces.  Under a hypothesis of coercivity we have continuum well-posedness, even for nonlinear operators.  Then we consider the meaning of ``conforming FE methods'' for such problems, and prove an \emph{a priori} bound on FE norm error which extends the Falk \cite{Falk1974} technique for VIs.  Then we provide a classical \emph{a posteriori} error estimator for PDEs, considering its application in the computed inactive set.

Consider a bounded, open domain $\Omega \subset \RR^d$, $d=2,3$, with Lipschitz boundary \cite{Ciarlet2002}.  We assume that $\Omega$ is polygonal or polyhedral.  Let $\cX = W^{1,p}(\Omega)$, $p>1$, be the Sobolev space of measurable functions with $p$th-integrable gradients \cite{Evans2010}.  We will assume continuous problem data, allowing well-defined point values, so suppose $\psi \in \cX \cap C(\bar\Omega)$ and $g\in C(\partial \Omega)$ satisfy $g \ge \psi|_{\partial\Omega}$.  As for the classical obstacle problem \eqref{eq:classical:admissible}, define the admissible subset
...
Let $\cX'$ be the dual space of $\cX$, with the application of $\omega \in \cX'$ to $v\in \cX$ denoted as $\omega[v] \in \RR$.  The norm on $\cX$ is denoted $\|\cdot\|$, while for $\cX'$ it is $\|\omega\|' = \sup_{\|v\|=1} |\omega[v]|$.  Let $F:\cK \to \cX'$ be a given operator, generally nonlinear, and let $\ell\in \cX'$ be given.  The VI associated to this data, a unilateral obstacle problem, is to find $u\in \cK$ so that
\begin{equation} \label{eq:vi}
F(u)[v - u] \ge \ell[v - u] \quad \text{ for all } v \in \cK.
\end{equation}

The VI problems considered here can be analyzed within the framework of coercivity and Lipschitz continuity.  We say $F$ is $q$-coercive, $1<q<\infty$, if there is $\alpha>0$ so that
\begin{equation} \label{eq:coercive}
(F(v) - F(w))[v - w] \ge \alpha \|v-w\|^q
\end{equation}
for all $v,w \in \cK$.  Note that if $F$ is $q$-coercive then it is also strictly monotone: $(F(v) - F(w))[v - w] > 0$ for $v\ne w$.  Let $B_R$ be the origin-centered ball in $\cX$ of radius $R>0$.  We say $F$ is Lipschitz on bounded subsets if there is $C(R)>0$ so that
\begin{equation} \label{eq:lipschitz}
\|F(v)-F(w)\|' \le C(R) \|v-w\|
\end{equation}
for all $v,w \in B_R\cap \cK$.  If $F$ satisfies \eqref{eq:lipschitz} then it is continuous.

From the strict monotonicity and continuity of $F$ it follows that a unique solution to \eqref{eq:vi} exists \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.

\begin{example}  \label{example:classicalobstacle}
In the classical obstacle problem, VI \eqref{eq:classical:vi} over $\cX=H^1(\Omega)=W^{1,2}(\Omega)$,
\begin{equation} \label{eq:classical:bilinearform}
F(u)[v] = a(u,v) = \int_\Omega \grad u\cdot \grad v\,dx
\end{equation}
is in fact bilinear.  It is $2$-coercive because the Laplacian is uniformly elliptic \cite{Evans2010}, and it is Lipschitz over $\cX$ with constant $C=1$.  While this $F$ is defined on all of $\cX$, $F$ in \eqref{eq:vi} might be only defined on $K$; Section \ref{sec:app} provides such an example operator.
\end{example}

Returning to the general case, note that the residual would be zero almost everywhere ($F(u)-\ell=0$) if the inequality constraint were absent, namely in the PDE case.  However, for a VI, over $\cK$ defined by \eqref{eq:admissible}, we only know that $F(u)-\ell=0$ a.e.~within the unknown inactive set $I_u$.  The residual $F(u)-\ell\in \cX'$ may be highly-irregular in the active set $A_u$, but it is nonnegative everywhere.  The following lemma states the (weak) complementarity property associated to VI \eqref{eq:vi}; compare the strong-form complementarity problem form, e.g.~\eqref{eq:classical:ncp} for \eqref{eq:classical:vi}.

\begin{lemma} \cite[Theorem II.6.9]{KinderlehrerStampacchia1980}.  Suppose $u\in \cK$ solves \eqref{eq:vi}.  Then $F(u)-\ell=d\mu_u$ is a positive Radon measure supported in the active set $A_u$, so for $w\in\cX$ we have
\begin{equation}
...
Let $\cT_h$ be a shape-regular mesh partition (triangulation, etc.) of $\Omega$ \cite{AinsworthOden2000,ElmanSilvesterWathen2014}.  Let $\cX_h \subset \cX \cap C(\bar\Omega)$ be a conforming finite-dimensional FE subspace over $\cT_h$.  Here $\cX_h = P_k(\cT_h)$, a Lagrange space, is most common; our examples will be piecewise-linear ($P_1$).  We will assume that there is $g_h\in\cX_h$ such that $g_h=g$ exactly along $\partial \Omega$.  Let $\psi_h \in \cX_h$ be the FE obstacle, which satisfies the compatibility requirement $\psi_h \le g_h$ along $\partial\Omega$.  Define the (nonempty) FE admissible set
...
Because $\cK_h \subset \cX_h \subset \cX$, we may regard \eqref{eq:fe:vi} as a conforming FE method for \eqref{eq:vi}.  However, this meaning is subtle for an obstacle problem, depending on the relationship between $\psi$ and $\psi_h$.  (Observe that no relationship between $\psi$ and $\psi_h$ was assumed in the previous paragraph, other than boundary compatibility with $g_h$.)  If $\psi_h$ is an interpolant of $\psi$, denoted $\psi_h = \Pi_h \psi$, or other projection, then $\cK_h \approx \cK$ in some sense.  There are three levels of ``conforming admissibility'':
\renewcommand{\labelenumi}{\emph{\roman{enumi})}}
\begin{enumerate}
\item $\cK_h \not \subset \cK$
\item $\cK_h \subset \cK$
...
\end{enumerate}
As noted early-on \cite[Figure 5.1.3]{Ciarlet2002}, situation \emph{i)} generally applies for an interpolated obstacle $\psi_h = \Pi_h \psi$, e.g.~for $\cX_h=P_1$ and when $\psi$ is not convex.  Situation \emph{ii)} evidently holds when $\psi_h \ge \psi$, which can be imposed by using a monotone injection operator, e.g.~``$\psi_h = R^\oplus \psi$'' in the notation from \cite{BuelerFarrell2024}; see also \cite{GraeserKornhuber2009}.  (For a related idea, see the proof of Theorem 5.1.2 in \cite{Ciarlet2002}.)  The strongest condition \emph{iii)} holds if $\psi_h=\psi$ exactly, for example when $\psi=0$ in the porous dam problems considered by \cite{AinsworthOdenLee1993}.

In any case, note that the obstacle $\psi$ is given \emph{as data} for VI problem \eqref{eq:vi}.  Exact values of $\psi$ can be evaluated, as needed, to represent the numerical solution within the numerical active set $A_u^h$.  To make an analogy, in a conforming FE approximation of the Poisson equation $-\grad^2 u = f$, the load-vector integrals using the source function (problem data) $f$ are assumed to be computed exactly.  In fact, quadrature is usually required, but even then exact evaluation at quadrature points is generally assumed.  Analogously for obstacle problems, exact values of $\psi$ are assumed to be available, anywhere as needed.  Thus, over elements of the mesh where there is believed to be active-set agreement, $K \subset A_u \cap A_u^h$ for $K\in\cT_h$, the difference (error) $e=u-u_h$ might be addressed in post-processing by improved representation of the data $\psi$.  This contrasts with any need to improve the mesh and/or solver generally so as to reduce $\|e\|$, and it also constrasts with any effects from mis-locating the free boundary.  For non-degenerate obstacle problems, accurate location of the free boundary, definitely including accurate solution of the interior condition within the inactive set, will reduce the sizes of the disagreement sets $I_u \cap A_u^h$ or $A_u \cap I_u^h$, but refinement deeper within the stabilized active set will play no role.
...
Theorem \ref{thm:genfalk} can be extended to non-conforming operators $F_h\approx F$, which adds a fourth term ``$(F(u_h)-F_h(u_h))[u_h]$'' to the bound \cite[Theorem 6.3]{Bueler2024}.  However, we will not need this extended result.

In situation \emph{ii)} earlier, namely $\cK_h\subset \cK$, the estimate \eqref{eq:falk} simplifies.
...
That is, the infimum over $\cK$ term in \eqref{eq:falk} is zero.  Furthermore, in the unconstrained PDE case where $A_u$ is of $d\mu_u$-measure zero, the bound can be reduced to Cea's lemma for $q$-coercive operators, namely $\|u-u_h\| \lesssim \inf_{v_h\in\cK_h} \|v_h - u\|^{1/(q-1)}$.  As is standard in FE theory, $\inf_{v_h\in\cK_h} \|v_h - u\|$ can be addressed by a regularity theory for the exact solution $u$, and interpolation theory \cite{AinsworthOden2000,ElmanSilvesterWathen2014}, giving convergence \cite[Theorem 5.1.2]{Ciarlet2002}.

However, how should we interpret the term
	$$(\ast) \qquad \inf_{v_h\in\cK_h} \int_{A_u} (v_h-\psi)\,d\mu_u$$
in \eqref{eq:falk} and \eqref{eq:falkconforming}?  It is unrelated to the FE solution $u_h$.  If $v_h < \psi$ is allowed then it might be negative, but then situation \emph{i)} holds and the ``$\inf_{v\in\cK}$'' term in \eqref{eq:falk} might counter-act (i.e.~if $v\ge \psi > u_h$ anywhere).

In the simpler situation \emph{ii)}, where $\psi_h\ge \psi$ and $\cK_h\subset \cK$, term $(\ast)$ is at least nonnegative.  It might be large because no $v_h\in\cK_h$ is close to $\psi$ over $A_u$, for instance where $v_h\ge \psi_h \gg \psi$.  On the other hand, it might be large even where $v_h-\psi$ can be made small over $A_u$, that is, if the source functional $\ell$ causes the residual $F(u)-\ell\ge 0$ to be large in $A_u$.  (Meaning: $d\mu_u(S)$ is positive and large for some measurable $S\subset A_u$.)  In summary, supposing $\cK_h\subset \cK$, bound \eqref{eq:falkconforming} is significantly larger than that from Cea's lemma only when the FE functions $v_h$ are blocked by $\psi_h$ from descending close to the continuum obstacle $\psi$, or when the (exact) residual $d\mu_u$ is large in the (exact) active set $A_u$, or both.

Returning to the continuum problems \eqref{eq:vi}, we point out a qualitative property of certain unilateral obstacle problems.  Note that for classical problems \eqref{eq:classical:vi} with continuous sources $f\in C(\bar\Omega)$, in general it is possible for $f(x)>0$ to occur in the active set.  That is, the force may be locally upward but insufficient to lift $u$ above a concave $\psi$.  By contrast, in certain reasonably-common situations this does not occur; the hypotheses of the following Lemma hold for \eqref{eq:classical:vi} if $\psi=0$.
...
\begin{proof}
Let $S\subset A_u$ be a Borel set, and denote its indicator function by $\chi_S$.  Note that $F(u)[\chi_S]=F(\psi)[\chi_S]=F(0)[\chi_S]$ is zero.  Thus by Lemma \ref{lem:measure},
\begin{equation*}
0 \le d\mu_u(S) = F(u)[\chi_S]-\ell[\chi_S] = 0 - \ell[\chi_S] = -\int_S f\,dx. \qedhere
\end{equation*}
\end{proof}

By definition, we say that unilateral obstacle problems based on $\ell[v] = \int_\Omega fv\,dx$ with $f\in C(\bar \Omega)$ have the \emph{blistering property} if $A_u \subset \{x \in \Omega\, :\, f(x)\le 0\}$.  To explain, the contrapositive says that if $f(x)>0$ anywhere then the inactive set $I_u$ must be non-empty, and $x\in I_u$; even a small upward force ``blisters'' the membrane off the obstacle.

Any classical problem \eqref{eq:classical:vi} with a smooth obstacle $\psi\in C^2(\bar\Omega)$ can be transformed into one having the blistering property.  To see this, transform the admissible set by $\tilde v=v-\psi$: $\tilde\cK = \{\tilde v\in\cX\,:\,\tilde v\ge 0 \text{ and } \tilde v|_{\partial\Omega}=g-\psi|_{\partial\Omega}\}$.  Then integration-by-parts shows that \eqref{eq:classical:vi} is equivalent to finding $\tilde u = u -\psi \in \tilde \cK$ so that
...
\end{equation}
For \eqref{eq:classical:vi:blister}, $x\in A_{\tilde u}$, i.e.~$\tilde u(x)=0$, implies that $\tilde f(x)= f(x)+\grad^2\psi(x)\le 0$.

FOR PROBLEMS WITH BLISTERING PROPERTY, NO FURTHER REFINEMENT IS NEEDED FOR ELEMENTS $K$ WHICH ARE 1) A POSTERIORI ACTIVE ON CURRENT MESH, 2) WHICH ARE CONTAINED IN $\{f<0\}$, AND 3) ARBITRARY REFINEMENT (AND CONVERGENCE TO $u$) IN THE COMPUTED INACTIVE SETS DOES NOT MOVE THE FREE BOUNDARY TO TOUCH $K$
