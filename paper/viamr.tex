...
In a unilateral obstacle problem the solution is constrained to be above (or below) a given function, while solving a partial differential equation (PDE) whereever it is not in contact.  Such a problem is posed in weak form as a variational inequality (VI) \cite{KinderlehrerStampacchia1980}, extending the PDE model from only using fixed-location boundary conditions to allowing for free boundaries \cite{Friedman1982}.  The solution and the free-boundary geometry must be determined simultaneously in such a problem.  Solvers are often built on constrained-optimization ideas, and in any case such problems are nonlinear even when stated for a linear operator.  Obstacle problems appear in many scientific, engineering, and economic applications \cite{FacchineiPang2003,KinderlehrerStampacchia1980,Rodrigues1987}.

The finite element (FE) method was adapted to the solution of VI and obstacle problems by the 1970s, for example see the \emph{a priori} estimate in \cite{Falk1974} or the well-known FE textbook by Ciarlet \cite{Ciarlet2002}.  The literature of FE solutions to VIs is now large, and we address some this literature, concerning \emph{a posteriori} estimators in particular, later in this Introduction.  However, VI problems continue to represent challenging problems for solvers and meshing methods; substantial developments have occurred in recent years, including \cite{BuelerFarrell2024,KeithSurowiec2024,KirbyShapero2024,PapadopoulosHintermuller2026}.

This paper introduces the VIAMR\footnote{Download or clone the library from \href{https://github.com/StefanoFochesatto/viamr}{\texttt{github.com/StefanoFochesatto/viamr}}.} Python library for adaptive mesh refinement (and mesh adaptation; see section \ref{sec:viamr}) on bilateral, scalar obstacle problems.  The library exploits the Firedrake \cite{Rathgeberetal2016} and PETSc \cite{petsc-user-ref} libraries for FE infrastructure and VI solvers, respectively.  In this Introduction we state the general cases to which the new library is addressed, we review some literature of the \emph{a posteriori} estimators, and we summarize the methods implemented in the library.

The unilateral Laplacian obstacle problem is the classical and canonical case, appearing in textbooks  \cite{Bueler2021,Ciarlet2002,Evans2010,KinderlehrerStampacchia1980} for example.  It finds the equilibrium vertical displacement $u$ of an elastic membrane, restricted to small deflections, over a domain $\Omega \subset \RR^d$.  The membrane, attached with displacement $g$ at the fixed boundary $\partial\Omega$, is subjected to an applied force $f$, but it is also constrained to be above a given obstacle $\psi$.  Thus the strong formulation is a complementarity problem \cite{FacchineiPang2003} over $\Omega$:
...
Ignoring regularity for the moment, and again speaking intuitively, integration of \eqref{eq:classical:vi} by parts yields $\big<\sigma, v-u\big>\ge 0$ for the residual $\sigma = -\nabla^2 u - f$.  This inequality says that a vector $v-u$, which points from the solution $u$ into $K$, is at most $90^\circ$ away from $\sigma$.  In particular, if $u$ is in the interior $\mathring\cK$ then $\sigma$ must be zero, and thus $u$ will solve the Poisson equation.  Furthermore $E_u=\Omega, C_u=\emptyset, \Gamma_u = \emptyset$.  Thus \eqref{eq:classical:vi} generalizes the weak-form Poisson equation, as a PDE over $E_u=\Omega$.
...
This general continuum VI problem is defined by certain data over a bounded, polygonal domain $\Omega\subset \RR^d$, including Dirichlet values $g\in C(D)\strut$, with well-behaved $D\subset\partial\Omega$, and two continuous functions $\lb,\ub \in C(\bar\Omega)$, the \emph{box constraints}.  These data must satisfy compatibility conditions:
\begin{libbox}
\begin{equation}
\lb < \ub \text{ on } \Omega \quad \text{and} \quad \lb \le g \le \ub \text{ on } D.\label{eq:compat}
\end{equation}
\end{libbox}
\noindent Fixed Neumann boundary conditions, if any, are assumed to be part of the source term $\ell$ used in \eqref{eq:vi} below.

The data may be already be in FE spaces, or as Unified Form Language (UFL) symbolic expressions \cite{Alnaesetal2014}, but we assume continuity as a convenience to enable nodal evaluation.  The library implements unilateral constraints by setting the missing constraint to Python \texttt{None}.
...
\end{equation}
\end{libbox}
\noindent where $F:\cK \to \cX'$ is the \emph{operator} and the \emph{source} $\ell \in \cX'$ is separated for clarity.\footnote{Separating $\ell$ from the nonlinear operator $F$ is not strictly needed, but it helps when considering how qualitative behavior relates to the data, and when comparing to the classical case \eqref{eq:classical:vi}.}  For example, $\left<F(u),w\right> = \int_\Omega \nabla u \cdot \nabla w$ and $\ell(w)=\int_\Omega f w$ in classical problem \eqref{eq:classical:vi}.  We assume $F$ is continuous, but it is generally nonlinear, and note that it may be defined only for admissible inputs.

Let us say a bit more about continuum VI \eqref{eq:vi} before addressing its FE solution.  For $\cX = W^{1,p}(\Omega)$, this VI is well-posed if $F$ satisfies certain conditions from the theory of monotone operators.  Specifically, if $F$ is strictly monotone, coercive, and continuous then it has a unique solution \cite[Corollary III.1.8]{KinderlehrerStampacchia1980}.  These conditions apply to the classical problem \eqref{eq:classical:vi} and its $p$-Laplacian extensions \cite{LiuBarrett1994}.  They also apply to the advection-diffusion problem in section \ref{sec:results}, and, with appropriate adjustments \cite{JouvetBueler2012}, to flat-topography cases of the highly-nonlinear glacier geometry problem in section \ref{sec:app}.  However, as for any solver library, the user should supply a well-posed problem.

Now define the \emph{lower-} and \emph{upper-contact} (active) subsets, the \emph{inactive} set, and the \emph{free boundary}, respectively:
\begin{gather}
...
\end{gather}
The contact sets are disjoint ($\Cbotu \cap \Ctopu = \emptyset$) by the compatibility conditions \eqref{eq:compat}; Figure \ref{fig:contactsets} illustrates a possibility.  The inactive set may be disconnected even when $\Omega$ is connected.  The free boundary generally includes disjoint components coming from lower and upper contact.
...
FIXME move stuff between here and start of section \ref{sec:inactive} to methods section (section \ref{sec:viamr}) and to results section \ref{sec:results}
