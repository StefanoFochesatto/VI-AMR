...
plt.figure()
meth = "VCD+BR"
plt.loglog(intvals(meth, "NE"), 1.0 - floatvals(meth, "JACCARD"), 'ko', ms=ms0+4, markerfacecolor="w", markeredgecolor="k", label=meth) # actually 2nd
meth = "UDO+BR"
plt.loglog(intvals(meth, "NE"), 1.0 - floatvals(meth, "JACCARD"), 'ko', ms=ms0, label=meth) # actually 1st
meth = "AVM"
plt.loglog(intvals(meth, "NE"), 1.0 - floatvals(meth, "JACCARD"), 'kd', ms=ms0+2, label=meth)
plt.loglog(intvals("UNI", "NE"), 1.0 - floatvals("UNI", "JACCARD"), 'r+', ms=ms0+2, label="uniform")
plt.grid(True)
